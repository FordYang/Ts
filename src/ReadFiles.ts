...
// fs.writeFileSync(path.resolve(__dirname, "file.bin"), b2);

let filep = path.resolve("./build/file.bin");
let content = fs.readFileSync(filep).toString();
...
for (let file of files)
{
    ...
    if (content)
    {
        ...
        let tmpname = path.resolve(name);//tmppath, 
        let tmpp1 = path.dirname(tmpname);

        if (!fs.existsSync(tmpp1))
        {
            fs.mkdirSync(tmpp1, {recursive:true});
        }
